...
class HumanMovementRandomiser(object):
    ...
    def update(self,position_list,boundary):
...
class SODA():
    ...
    def room_setup(self, min_humans, min_wandHumans, min_plants, min_tables, min_relations, max_humans, max_wandHumans, max_plants, max_tables, max_relations):
        ...
        # print("TYPE")
        # print(self.walls_data[0][0][0])
        # print(type(self.walls_data[0][0][0]))
        ...
        # print("WALL TYPE", type(self.walls[0]))
        self.data['walls'] = []
        ...
        for child in self.coppelia.get_objects_in_tree():
            ...
            if name == 'camera_fr':
                ...
                # if perspective_angle!=xx:
                #     vision_sensor.set_perspective_angle(angle)
                # if resolution!=xx:
                #     vision_sensor.set_resolution([x,y])
                # if window_size!=xx:
                #     vision_sensor.set_resolution((x,y))
                # if entity_to_render!=x:
                #     vision_sensor.set_entity_to_render(handle_of_object or -1 for all)
                # if far_clipping_plane!=xx:
                #     vision_sensor.set_far_clipping_plane(metres)
                # if near_clipping_plane!=xx:
                #     vision_sensor.set_near_clipping_plane(metres)
        ...
        #print("WANDERING HUMANS")
        #print(n_wandering_humans)
        # for h in [a, b]:
        ...
        print("INTERACTIONS", self.interacting_humans)
    ...
    def soda_compute(self,people, objects, walls, goals):
        ...
        for h in self.wandering_humans:
            h.update(self.position_list,self.boundary)
