...
if __name__ == '__main__':
    ...
    # Remote object connection for OmniRobot
    try:
        proxyString = ic.getProperties().getProperty('OmniRobot1Proxy')
        try:
            basePrx = ic.stringToProxy(proxyString)
            omnirobot1_proxy = RoboCompOmniRobot.OmniRobotPrx.uncheckedCast(basePrx)
            mprx["OmniRobotProxy1"] = omnirobot1_proxy
        except Ice.Exception:
            print('Cannot connect to the remote object (OmniRobot)', proxyString)
            #traceback.print_exc()
            status = 1
    ...
    try:
        controlType = ic.getProperties().getProperty('ControlType')
        MouseControl = ic.getProperties().getProperty('MouseControl')
    except Ice.Exception:
    ...
    if status == 0:
        worker = SpecificWorker(mprx, controlType, MouseControl)
